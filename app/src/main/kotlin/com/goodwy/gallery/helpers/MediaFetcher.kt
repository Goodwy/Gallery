...
class MediaFetcher(val context: Context) {
    ...
    fun groupMedia(media: ArrayList<Medium>, path: String): ArrayList<ThumbnailItem> {
        ...
        val sorted = if (currentGrouping and GROUP_BY_LAST_MODIFIED_DAILY != 0 || currentGrouping and GROUP_BY_LAST_MODIFIED_MONTHLY != 0 ||
            ...
            currentGrouping and GROUP_BY_DATE_TAKEN_YEARLY != 0 || currentGrouping and GROUP_BY_LAST_MODIFIED_YEARLY != 0
        ) {
        ...
        for ((key, value) in mediumGroups) {
            ...
            thumbnailItems.add(ThumbnailSection(sectionKey, value.size))
    ...
    private fun getFormattedKey(key: String, grouping: Int, today: String, yesterday: String, count: Int): String {
        var result = when {
            ...
            grouping and GROUP_BY_LAST_MODIFIED_YEARLY != 0 || grouping and GROUP_BY_DATE_TAKEN_YEARLY != 0 -> formatDate(key,
                showDay = false,
                hideMonth = true
            )
        ...
        if (result.isEmpty()) {
            result = context.getString(com.goodwy.commons.R.string.unknown)
        }
    ...
    private fun getFinalDate(date: String, today: String, yesterday: String): String {
        return when (date) {
            today -> context.getString(com.goodwy.commons.R.string.today)
            yesterday -> context.getString(com.goodwy.commons.R.string.yesterday)
            else -> date
        }
    }

    private fun formatDate(timestamp: String, showDay: Boolean, hideMonth: Boolean = false): String {
